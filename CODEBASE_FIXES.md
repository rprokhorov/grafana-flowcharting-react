...
> **Статус (обновлено 2026-06-06):** **все пункты P0, P1 и P2 проработаны.**
> P0/P1 — 12 «чистых» багов (с хешами коммитов) + 5 фич (LinkMap, CSV-источник,
> date thresholds, map options в UI, EventMap/TextMap modes). P2 — минификация
> бандла (1.84 → 0.67 MiB), opt-out CDN + CSP-доки, безопасность openDrawioEditor,
> реиндексация thresholds, сужение catch в renderer, согласование editor с типами,
> и тесты на основные gaps. Открытым остаётся лишь авто-тест на flowchart index
> shrink и build-валидацию логотипа (проверены вручную).
...
- [x] Доделать EventMap: большая часть настроек не работает или не настраивается. (реализовано)
...
- [x] Доделать TextMap modes. (реализовано)
...
- [x] Уменьшить размер сборки. (минификация: 1.84 MiB → 667 KiB)
...
- [x] Пересмотреть `eval` и CDN fallback с точки зрения deployment/security. (CDN opt-out + CSP docs)
...
- [x] Сделать `FlowchartsEditor.openDrawioEditor()` безопаснее. (origin + listener cleanup)
...
- [x] Реиндексировать threshold levels после удаления. (реализовано)
...
- [x] Не скрывать реальные ошибки renderer-а общим catch. (сужено до missing-stencil)
...
- [x] Согласовать options editor с типами. (column/invert/hidden + map options + text/event modes)
...
- [x] Добавить тесты на уже найденные gaps. (частично — основные покрыты)
  Покрыто: regex `/g`/`/y` (spec/regexCache), stale style reset + LinkMap (spec/xcell),
  multiple matched metrics (spec/ruleengine), date thresholds (spec/rule),
  encode/decode round-trip (spec/drawio-codec), CSV-источник / thresholds / mapping /
  tooltip / navigator / robustness (e2e). Остаётся как бэклог: flowchart index shrink
  и build-валидация логотипа — проверены вручную, без автотеста.
